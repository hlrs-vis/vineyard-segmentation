...
supported_formats = [f".{fmt.upper()}" for fmt in config['Dataset']['supported_formats']]
...
# Create a dictionary to map RGB values to integer labels
rgb_to_label = {tuple(value): idx for idx, (key, value) in enumerate(colormap.items())}

# Example label (dummy data for demonstration)
label = single_patch_mask  # Replace with actual data

def rgb_to_2D_label(label):
    label_seg = np.zeros(label.shape[:2], dtype=np.uint8)
    for rgb, idx in rgb_to_label.items():
        label_seg[np.all(label == rgb, axis=-1)] = idx
    return label_seg
